...
class MuxHandler(BaseHTTPRequestHandler):
    ...
    def _send_response(self, status: int, body: str, extra_headers: dict | None = None):
        encoded = body.encode()
        # Detect SSE format and set correct Content-Type
        is_sse = body.strip().startswith("event:") or body.strip().startswith("data:")
    ...
    def _send_json(self, status: int, body: str, extra_headers: dict | None = None):
        self._send_response(status, body, extra_headers)
    ...
    def do_GET(self):
        # Block OAuth discovery endpoints — we handle auth internally
    ...
    def do_POST(self):
        ...
        # Never forward 401 to Claude — means upstream session expired, reinitialize
        if status == 401:
            new_sid = _upstream_initialize()
            if new_sid:
                status, resp_headers, body = _forward_to_upstream(raw, new_sid)
                if "Mcp-Session-Id" in resp_headers:
                    extra["Mcp-Session-Id"] = resp_headers["Mcp-Session-Id"]
