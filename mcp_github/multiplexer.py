...
def _wait_for_upstream(retries: int = 15, delay: float = 4.0) -> bool:
    """Try every 4 seconds for up to 60 seconds."""
    print(f"Waiting for upstream on port {UPSTREAM_PORT} (max {retries * delay:.0f}s)...", flush=True)
    for i in range(1, retries + 1):
        try:
            ...
            with urllib.request.urlopen(req, timeout=3):
                print(f"Upstream ready after attempt {i}/{retries}", flush=True)
                return True
        except urllib.error.HTTPError as e:
            # 401 means server is up but requires auth — that's fine, it's ready
            if e.code == 401:
                print(f"Upstream ready (HTTP 401 = auth required) after attempt {i}/{retries}", flush=True)
                return True
            print(f"Attempt {i}/{retries}: HTTP {e.code}, retrying in {delay:.0f}s...", flush=True)
