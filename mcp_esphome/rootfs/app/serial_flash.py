...
--before no_reset, so it never triggers that reset in the first place.
...
this same troubleshooting session); --before no_reset only means it
...
async def flash_factory_bin(bin_path: Path, port: str, baud: int = 115200) -> dict:
    ...
    firmware.factory.bin at flash offset 0x0, with --before no_reset so
