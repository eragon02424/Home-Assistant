...
# Laufzeit-State (aenderbar via MCP Tools)
state = {
...
# ---------------------------------------------------------------------------
# Auto-Detect: erster ttyACM* oder ttyUSB*
# ---------------------------------------------------------------------------
...
def open_port(port: str, baud: int) -> bool:
    ...
    try:
        ...
        s.dtr = False  # ESP32-S2: kein Reset beim Oeffnen
        s.rts = False
        with ser_lock:
...
def read_loop():
    """Liest dauerhaft vom offenen Port. Laeuft im Background-Thread."""
    while True:
        with ser_lock:
            s = ser
        if s and s.is_open:
            try:
                ...
                # Port verschwunden (ESP schlaeft)
                with ser_lock:
                    try:
                        ser.close()
                    except Exception:
...
# ---------------------------------------------------------------------------
# pyudev Watcher: ESP32-S2 Enumeration erkennen
# ---------------------------------------------------------------------------
def udev_watcher():
    ...
    for device in iter(monitor.poll, None):
        if device.action == "add":
            ...
            if dev_node and ("ttyACM" in dev_node or "ttyUSB" in dev_node):
                target = state["active_port"] or "auto"
                if target == "auto" or target is None or dev_node == target:
                    print(f"[udev] Neues Device: {dev_node} — oeffne...", flush=True)
                    time.sleep(0.1)  # kurz warten bis CDC ready
...
def handle_serial_read_timerange(params: dict) -> dict:
    since = params.get("since")   # ISO string
    until = params.get("until")   # ISO string, optional
...
def handle_serial_list_ports(params: dict) -> dict:
    ...
    try:
        for name in os.listdir("/dev/serial/by-id"):
            ...
            by_id[name] = os.path.normpath(
                os.path.join("/dev/serial/by-id", target))
    except Exception:
        pass
...
def handle_serial_status(params: dict) -> dict:
    ...
    port_name = s.port if s else None
    with buffer_lock:
...
TOOL_DEFS = [
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "until": {"type": "string", "description": "Ende ISO timestamp (optional, default = jetzt)"},
    ...
    },
    {
        ...
        "description": "Wechselt den aktiven seriellen Port (live, kein Add-on Neustart). Ueberschreibt Auto-Detect.",
        "inputSchema": {
            "type": "object",
            "required": ["port"],
            "properties": {
    ...
    },
    {
        ...
        "description": "Aendert die Baudrate live (kein Neustart). Gilt sofort auf offenem Port.",
        "inputSchema": {
            "type": "object",
            "required": ["baud_rate"],
            "properties": {
    ...
    {
        "name": "serial_status",
        "description": "Aktueller Status: Port, Baudrate, ob offen, Buffer-Fuellstand, Log-Groesse.",
        "inputSchema": {"type": "object", "properties": {}}
    },
]
...
async def main():
    ...
    # Beim Start: auto-detect
    port = auto_detect_port()
    if port:
        state["active_port"] = port
        open_port(port, state["baud_rate"])
    else:
        print("[serial] Kein Port gefunden beim Start — warte auf udev...", flush=True)
